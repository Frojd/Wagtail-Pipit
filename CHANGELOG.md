...
## [Unreleased]
### Added
- Add @testing-library/dom as explicit devDependency, jest-dom 7 requires it as a peer and it was only present because storybook depends on it (@marteinn)
- Add AGENTS.md and CLAUDE.md to frontend, generated by next dev in Next.js 16.3 (@marteinn)
...
- Upgrade @testing-library/jest-dom to 7.0.1, which raises the minimum Node version to 22 (@marteinn)
...
- Align dev and CI on postgis 16-3.5-alpine and pin the db service to linux/amd64, postgis publishes no arm64 image (@marteinn)
